...
exports.list = (req, res) => {
  try {
    ...
    if (req.query.employee_id) {
      ...
      all = all.filter(x => keys.some(k => String(x?.[k] ?? '').toLowerCase() === eid));
    }
...
exports.create = (req, res) => {
  ...
  if (!Number.isInteger(oh) || oh < 1 || oh > 24) return err(res, 'Overtime hours must be a whole number between 1 and 24');
  const rm = parseFloat(overtime_rate_multiplier || 1.5);
...
exports.update = (req, res) => {
  ...
  const rm = parseFloat(req.body.overtime_rate_multiplier || 1.5);
  const emp = db.employees?.find(e => e.id === req.body.HRMS_employee_id);
  const hourlyRate = emp ? (parseFloat(emp.Last_Drawn_Salary) || 50000) / (30 * 8) : 100;
  const overtime_amount = parseFloat((oh * hourlyRate * rm).toFixed(2));
